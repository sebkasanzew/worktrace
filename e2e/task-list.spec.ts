...
test.describe("Task List UI", () => {
    ...
    test("should display task list header with title and controls", async ({
        page,
    }) => {
    ...
    test("should display issue details correctly", async ({ page }) => {
        ...
        await expect(
            page.getByText(/Assigned to: Test User/).first(),
        ).toBeVisible();
    ...
    test("should display different statuses for different issues", async ({
        page,
    }) => {
        // Wait for issues to load
        await expect(page.getByText("TEST-123")).toBeVisible();
    ...
    test("should show empty state when no issues", async ({ page }) => {
        ...
        await expect(
            page.getByText("No unresolved issues assigned to you."),
        ).toBeVisible();
    ...
    test("should display error state when API fails", async ({ page }) => {
        ...
        await expect(
            page.getByText(/Failed to fetch issues: Network error/),
        ).toBeVisible();
    ...
    test("should format dates correctly", async ({ page }) => {
        ...
        const dateText = await page.getByText(/Updated: \d{1,2}\/\d{1,2}\/\d{4}/).first().textContent();
