...
interface DayWorklogsDialogProps {
  open: boolean
  onOpenChange: (open: boolean) => void
  date: Date
}

export function DayWorklogsDialog({ open, onOpenChange, date }: DayWorklogsDialogProps) {
  const { t } = useTranslation()
  const queryClient = useQueryClient()
  const dateStr = format(date, "yyyy-MM-dd")
  const { data, isLoading, isFetching } = useUserWorklogsByDateRange(dateStr, dateStr)
  ...
  const handleRefresh = () => {
    queryClient.invalidateQueries({ queryKey: jiraKeys.userWorklogs(dateStr, dateStr) })
  }
      ...
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          <div className="flex items-center justify-between">
            <DialogTitle>{t("Worklogs for {{date}}", { date: format(date, "PP") })}</DialogTitle>
            <Button
              variant="ghost"
              size="sm"
